<div class="container">
    ...
    <div class="intro-content swib-maintext my-5">      
        ...
        <div class="table-responsive text-left">
            ...
            <h5>The first step in conducting an analysis is to upload the input data file to be used in SNOSWAB. The users can run SNOSWAB either by using the pre-loaded sample dataset or by uploading a new dataset.</h5>
            ...
            <h5>For testing SNOSWAB and better understanding how the various components of the tool work, the user can upload the test data set provided by clicking on “Try the tool using the test dataset” button. The test dataset contains three years of weather and calibration data. The calibration data consists of snow layer thickness (cm; UCD1; measured at a weather station included in the Meteorological Services of Canada [Environment and Climate Change Canada] monitoring network), soil water content (%; UCD2; measured in a research field), surface runoff (mm; UCD3; obtained from hydrograph separation of stream discharges obtained from a stream gaging station included in the Water Survey of Canada [Environment and Climate Change Canada] monitoring network) and groundwater recharge(mm; UCD4; obtained based on measured changes in water table levels in a research well).</h5>

            <h5>For using SNOSWAB the users need to upload time series datasets. The tool accepts source data sets in Comma Separated File (csv) format. The users can use the “Export Input Data - Daily” menu to obtain a correctly formatted input file that can be used as a model for populating the input data file with user data. The user input file can be uploaded to SNOSWAB by using the “Upload user data” button. It should be noted that the tool cannot accommodate missing data (i.e., blank rows in required data columns) or erroneous data entries, and hence it is recommended that the integrity of the source data is verified before uploading.</h5>

            <h5>The input data file consists of a tabular file with 1 column dedicated to calendar date, 4 columns dedicated to required input data (TEMP - daily mean daily air temperature; TOTPP - daily total precipitation; RAIN - daily rain; ETA - daily actual (or crop) evapotranspiration) and 5 columns reserved for optional calibration data (UCD1 to UCD5). The required input data columns have to contain values in all rows, while the optional data columns can be left blank if data is not available. The calibration data sets are not restricted to certain parameters and can include time series for any parameter that the user wants to use for comparing with the output from SNOSWAB.  Examples of calibration time series datasets include thickness of snow layer, soil water content, groundwater recharge, surface runoff, etc.</h5>
